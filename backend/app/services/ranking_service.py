...
def get_doctor_real_metrics(db: Session, doctor: Doctor, user_lat: float, user_lng: float) -> dict:
    """Calculate real database-driven metrics for an approved doctor."""
    # 1. Distance
    distance_km = (
        calculate_distance_km(user_lat, user_lng, doctor.latitude, doctor.longitude)
        if user_lat is not None and user_lng is not None else None
    )

    # 2. Real Queue counts from database
    ...
    # 3. Real Reviews and average rating
    ...
    if avg_rating:
        ...
        reasons.append("✓ New verified clinic practitioner")
    ...
    if is_open:
        ...
        reasons.append("⚠️ Clinic closed for the day")
...
def rank_approved_doctors(
    db: Session,
    specialty: str = "",
    user_lat: float | None = DEFAULT_USER_LATITUDE,
    user_lng: float | None = DEFAULT_USER_LONGITUDE,
    priority: str = "best_match",
    max_fee: float | None = None,
    max_distance: float | None = None,
    only_open: bool = False,
    min_rating: float | None = None
) -> list:
    """Rank ONLY registered, approved doctors from the real database."""
    query = db.query(Doctor).filter(Doctor.account_status == AccountStatusEnum.APPROVED)
    ...
    for doc in approved_doctors:
        ...
        # Filters
        if max_distance and metrics["distance_km"] > max_distance:
            continue
    ...
    max_doc_fee = max([e["doctor"].fee for e in enriched] + [1000.0])

    weights = {
        "specialty": 0.3, "distance": 0.2, "wait": 0.2,
        "rating": 0.15, "fee": 0.1, "availability": 0.05
    }
    ...
    for item in enriched:
        ...
        fee_score = max(0.0, 1.0 - (doc.fee / max_doc_fee))
        ...
        scored.append({
            ...
            "bio": doc.bio,
            "account_status": doc.account_status.value,
